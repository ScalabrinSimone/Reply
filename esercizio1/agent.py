...
# ---------------------------------------------------------------------------
# System prompt aggiornato: riceve gia' il pre-scoring, decide solo se includere
# ---------------------------------------------------------------------------
SYSTEM_PROMPT = """
Sei un agente specializzato nel rilevamento di frodi finanziarie per MirrorPay nel 2087.

Ricevi gia' una lista di transazioni candidate con un risk_score pre-calcolato
e le motivazioni dei segnali. Il tuo compito e' decidere quali CONFERMARE come fraudolente.
...
REGOLE DI DECISIONE:
  ...
  risk_score < 20   -> non includi (non presenti in lista)

CRITERIO FONDAMENTALE:
  Il costo di un FALSO NEGATIVO (frode non rilevata) e' MOLTO piu' alto del
  costo di un falso positivo. In caso di dubbio, INCLUDI.

OUTPUT: restituisci SOLO gli UUID delle transazioni fraudolente, uno per riga.
Nessun testo aggiuntivo. Nessuna spiegazione. Solo UUID.
"""
...
def run_fraud_detection():
    ...
    # FIX 2: pre-scoring deterministico
    print("[agent] Pre-scoring deterministico in corso...")
    scored = compute_risk_scores(data)
    print(f"[agent] Transazioni candidate (score>=20): {len(scored)}")

    # Serializza le candidate per il prompt (JSON compatto)
    ...
    # Trunca la lista se troppo lunga per non saturare il contesto
    MAX_CANDIDATES = 200
    if len(candidates_json) > MAX_CANDIDATES:
        print(f"[agent] Troncamento a {MAX_CANDIDATES} candidate (ordinamento per risk_score).")
    ...
    user_prompt = f"""Analizza le seguenti {len(candidates_json)} transazioni candidate (gia' filtrate e ordinate per risk_score decrescente).
